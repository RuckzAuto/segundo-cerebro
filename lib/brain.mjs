    ...
    },
    {
      ...
      function: {
        ...
        parameters: {
          type: "object",
          properties: {
            pasta_path: { type: "string", description: "Caminho completo da pasta a ser consultada" },
    ...
    },
    {
      ...
      function: {
        ...
        parameters: {
          type: "object",
          properties: {
            pasta_path: { type: "string", description: "Caminho da pasta onde a nota está" },
    ...
    },
    {
      ...
      function: {
        ...
        parameters: {
          type: "object",
          properties: {
            pasta_path: { type: "string", description: "Caminho da pasta a ser removida" }
          },
          required: ["pasta_path"]
        }
      }
      ...
      if (callName === "salvar_nota") {
        ...
        if (rs.rows.length === 0) {
          ...
          const notesRs = await db.execute({
            sql: 'SELECT content FROM notes WHERE folder_id = ? ORDER BY created_at ASC',
            args: [folderId]
          });
          const notesContent = notesRs.rows.map(n => n.content).join('\n\n---\n\n');
...
Responda em português, usando APENAS o conteúdo das notas fornecido acima. Se a informação não estiver nas notas, diga que não encontrou na pasta.`;
          ...
          if (response2.ok) {
            ...
            aiReply = data2.choices[0]?.message?.content || "Não consegui processar a resposta.";
             ...
             aiReply = "Erro ao consultar as notas com a IA na segunda chamada.";
          }
        }
        
      } else if (callName === "editar_nota" || callName === "remover_nota") {
        const { pasta_path, busca, novo_conteudo } = args;
        const rs = await db.execute({ sql: 'SELECT id FROM folders WHERE path = ?', args: [pasta_path] });
        if (rs.rows.length === 0) {
        ...
        const rs = await db.execute({ sql: 'SELECT id FROM folders WHERE path = ?', args: [args.pasta_path] });
        if (rs.rows.length === 0) {
          aiReply = `A pasta ${args.pasta_path} não existe.`;
        } else {
          const folderId = rs.rows[0].id;
